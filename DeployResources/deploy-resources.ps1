...
az datafactory create --factory-name $DataFactoryName --resource-group $ResourceGroupName --location $Location
...
for ($i = 1; $i -le 200; $i++) {
    ...
    az datafactory pipeline create --factory-name $DataFactoryName --pipeline $DataFactoryPipelineDefinitionFileName --name $PipelineName --resource-group $ResourceGroupName
}
